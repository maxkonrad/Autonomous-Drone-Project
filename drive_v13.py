...
TARGET_X = 2.0              # ➡️ X Ekseninde İlerleme Hedefi (2.0 Metre)
HOVER_DURATION = 5.0        # ⏱️ Havada İleri Hat Süzülme Süresi (5 Saniye)
...
class SimulatedMSP:
    def apply_rc_to_physics(self, roll, pitch, throttle, true_z):
        ...
        # Gerçek fırçasız itki izdüşümü: İleri gitme gücü ana dikey motor gücünden çalınır!
...
class AutonomousFlightController:
    def __init__(self):
        ...
        # İleri uçuş sapmalarını engellemek için Pitch ve Roll PID'lerine I-term (1.2) ekledik
...
with open("helion_v13_demo_log.txt", "w") as log_file:
    ...
    print("\n🚀 Helion 10 v13 - X Ekseninde İlerlemeli Demo Başlıyor!")
    ...
    try:
        for i in range(3000): 
            ...
            # Sinematik takip kamerası: Drone'u X ekseninde giderken net odaklar
            camera.set_pose(pos=np.array([3.0, -3.0, 2.0]), lookat=true_pos)
            scene.step()
            ...
            if mod == "KALKIS":
                ...
                if abs(filtered_pos[2] - TARGET_ALTITUDE) < 0.05:
                    mod = "ILERI_UÇUŞ"
                    hover_timer = 0.0
                    print(f"\n🎯 {TARGET_ALTITUDE}m'ye çıkıldı. X yönünde 2.0m hedefine ilerleme başladı.")
                    
            elif mod == "ILERI_UÇUŞ":
                # Yüksekliği koru ama X'i 2.0 metreye çek!
                target = [TARGET_X, 0.0, TARGET_ALTITUDE]
                
                hover_timer += dt
                # Hem 5 saniye geçecek hem de hedef X koordinatına kabaca yaklaşmış olacak
                if hover_timer >= HOVER_DURATION and abs(filtered_pos[0] - TARGET_X) < 0.15:
                    mod = "INIŞ_ALGORITMASI"
                    print("\n🪂 İleri uçuş rotası tamamlandı. Hedef koordinatta dikey iniş yapılıyor.")
                    
            elif mod == "INIŞ_ALGORITMASI":
                # Vardığı 2.0 metre koordinatında dikey iniş yap
            ...
            if i % 20 == 0:
                print(f"\r⏱️ {elapsed:04.1f}s | Mod: {mod:<15} | X: {filtered_pos[0]:.2f}m | Z: {filtered_pos[2]:.2f}m | Pitch: {pitch_pwm:.0f}", end="")

            if mod == "INIŞ_ALGORITMASI" and true_pos[2] < 0.12:
                print(f"\n\n🏁 Helion başarıyla X=2.0 metreye uçtu, havada süzüldü ve motor kapattı!")
                break 
                
    except Exception as e:
        print(f"\nUçuş esnasında hata: {e}")

print("\n✅ İlerlemeli demo başarıyla tamamlandı.")
